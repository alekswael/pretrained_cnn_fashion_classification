...
def input_parser(): # This is the function that parses the input arguments when run from the terminal.
    ...
    ap.add_argument("-bs",
                    ...
                    type = int, default=128) # This is the argument for the batch size.
    ap.add_argument("-po",
                    "--pooling_type",
    ...
    ap.add_argument("-e",
                    ...
                    type = int, default=1) # This is the argument for the number of epochs.
...
def setup_data(train_df, test_df, val_df, train_generator, val_generator, test_generator, batch_size_arg):
    # Split the data into three categories.
    train_ds = train_generator.flow_from_dataframe(
        dataframe=train_df.sample(n = 160, random_state=42),
        x_col='image_path',
        y_col='class_label',
        target_size=(224, 224),
        color_mode='rgb',
    ...
    val_ds = val_generator.flow_from_dataframe(
        dataframe=val_df.sample(n = 100, random_state=42),
        x_col='image_path',
        y_col='class_label',
        target_size=(224, 224),
        color_mode='rgb',
    ...
    test_ds = test_generator.flow_from_dataframe(
        dataframe=test_df.sample(n = 100, random_state=42),
        x_col='image_path',
        y_col='class_label',
        target_size=(224, 224),
        color_mode='rgb',
...
def main():
    ...
    train_ds, val_ds, test_ds = setup_data(train_df, test_df, val_df, train_generator, val_generator, test_generator, args.batch_size)
